...
export default function CounsellingPage() {
  ...
  const counsellingSessions = mockSessions.filter((s) => s.unit_name === "Counselling");
  const counsellingHead = mockUsers.find((u) => u.roles.includes("counselling_head"));
  const filtered = counsellingSessions.filter((s) => s.student_name.toLowerCase().includes(search.toLowerCase()));
  const completed = counsellingSessions.filter((s) => s.status === "completed").length;
  const booked = counsellingSessions.filter((s) => s.status === "booked").length;
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
      <motion.div variants={itemVariants}>
        ...
        <h1 className="text-4xl font-bold text-[#1A2B4A] tracking-tight">Counselling</h1>
        <p className="text-[#6B7280] mt-2 text-lg">Counselling sessions managed by Dr. Grace Asare</p>
      </motion.div>
      ...
      {counsellingHead && (
        <motion.div variants={itemVariants} className="rounded-2xl border bg-white p-6 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-[#1A2B4A]">{counsellingHead.full_name}</h2>
              <p className="text-sm text-[#6B7280]">Counselling Unit Head</p>
              <div className="flex items-center gap-4 mt-1 text-sm text-[#6B7280]">
                <span className="flex items-center gap-1"><Phone className="w-3.5 h-3.5" />{counsellingHead.phone}</span>
      ...
      <div className="grid gap-5 grid-cols-1 sm:grid-cols-3">
        {[
          { label: "Total Sessions", value: counsellingSessions.length, color: "text-[#1A2B4A]" },
          { label: "Completed", value: completed, color: "text-emerald-600" },
          { label: "Upcoming", value: booked, color: "text-blue-600" },
        ].map((item, i) => (
      ...
      <motion.div variants={itemVariants} className="rounded-2xl border bg-white shadow-sm overflow-hidden">
        <div className="p-6 border-b border-[#f3f4f6]">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-[#1A2B4A]">Counselling Sessions</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#f3f4f6] bg-[#f8f4ef]/50">
                {["Student", "Counsellor", "Date", "Location", "Status"].map((h) => (
        ...
        {filtered.length === 0 && <div className="p-16 text-center text-[#6B7280]">No counselling sessions found.</div>}
      </motion.div>
    </motion.div>
  );
}