...
export default function AdvisingDashboard() {
  ...
  const firstName = session?.user?.fullName?.split(" ")[0] ?? "Advisor";

  const { data, isLoading: loading, isFetching: refreshing, refetch } = useQuery({
    queryKey: ['advising-dashboard'],
    queryFn: async () => {
      const res = await fetch(`${API_URL}/support/advising/dashboard`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error("Failed to fetch advising dashboard");
      return res.json();
    },
    enabled: !!token,
    staleTime: 1000 * 60 * 5,
  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top"]}>
      ...
      <View style={styles.headerContainer}>
        <Text style={styles.greeting}>Academic Advisor</Text>
        ...
        <Animated.View entering={FadeInDown.delay(50).duration(400)}>
          <View style={styles.quickActions}>
            <TouchableOpacity
              ...
              onPress={() =>
                router.push(
                  `/support/schedule-session?asAdvisor=true&unitId=3` as any
                )
              }
            >
          ...
          {upcomingSessions.length > 0 ? (
            ...
            <View style={styles.emptyBlock}>
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push(`/support/schedule-session?asAdvisor=true&unitId=3` as any)
                }
              >
