...
export default function EditPostScreen() {
  ...
  const { id, type, preselectGroup } = useLocalSearchParams<{ id: string, type?: string, preselectGroup?: string }>();
  ...
  const { data: fetchedData, isLoading: isDataLoading } = useQuery({
    queryKey: ['editData', id, type],
    queryFn: async () => {
      const isEditEvent = type === 'event';
      const endpoint = isEditEvent ? `/events/${id}` : `/posts/${id}`;
      const res = await apiRequest<{ event?: any, post?: any }>(endpoint, {
        headers: { Authorization: `Bearer ${session?.accessToken}` },
      });
      return isEditEvent ? res.event : res.post;
    },
    initialData: () => {
      const isEditEvent = type === 'event';
      const cached = queryClient.getQueryData<any>(isEditEvent ? ['event', id] : ['post', id]);
  ...
  useEffect(() => {
    if (fetchedData) {
      ...
      if (isEditEvent) {
        ...
        if (data.reminderMinutes !== undefined && data.reminderMinutes !== null) {
        ...
        const matchedCategory = categories.find(c => c.toLowerCase() === data.category.toLowerCase());
        if (matchedCategory) {
          setCategory(matchedCategory);
        }
      }
      
      if (data.visibility === "targeted" && data.targets?.length) {
        setIsTargeted(true);
  ...
  const handleSubmit = async () => {
    ...
    const endpoint = isEvent ? `/events/${id}` : `/posts/${id}`;
    const payload: any = {
      title: title.trim(),
  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top"]}>
      ...
      >
        ...
        {isDataLoading && (
          <ActivityIndicator size="small" color="#A93C40" style={{ marginVertical: 10 }} />
        )}
